...
class Graph {
  ...
  dfsIterative(start){
    ...
    while (stack.length){
      ...
      adjacencyList[currVertex].forEach((neigh) => {
        if (!visited[neigh]){
          stack.push(neigh);
          visited[neigh] = true;
  ...
  breadthFirst(start){
    ...
    while (queue.length){
      ...
      this.adjacencyList[curV].forEach((neigh) => {
        if (!obVisited[neigh]){
...
const result = something('aliens', {a:1, b:2, c:3});
// const result = twoSum([2, 7, 11, 15], 13);
let t2 = performance.now();
output.innerHTML = result + '<br>' + (t2 - t1) + ' milliseconds';
